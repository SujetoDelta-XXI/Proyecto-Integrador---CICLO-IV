...
function TwoFactorSetup({ metodos, onSuccess, onCancel, registrarCorreoForzado }) {
  ...
  useEffect(() => {
    if (!registrarCorreoForzado && metodo === "correo" && metodos.correo && !enviado) {
      handleEnviarCodigo();
    }
  }, [metodo, metodos.correo, enviado, registrarCorreoForzado]);
  ...
  const handleRegistrarCorreo = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success("Correo registrado correctamente. Por favor revisa tu correo para validar.");
